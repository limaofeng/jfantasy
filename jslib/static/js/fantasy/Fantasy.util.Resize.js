//============================================================
//[描    述]  Fantasy.awt.Reszie		主要功能：页面元素的缩放效果
//============================================================
Fantasy.util.jClass(Fantasy.util.Observable,(function(){
	...
	return {
		...
		jClass : 'Fantasy.util.Resize',
		
		initialize : function($super,target, options) {
			$super(['resize','option']);
			
			this.target = $(target);//缩放对象
			...
			//获取边框宽度
			this._borderX = (parseInt(this.target.css('borderLeftWidth')) || 0) + (parseInt(this.target.css('borderRightWidth')) || 0);
			this._borderY = (parseInt(this.target.css('borderTopWidth')) || 0) + (parseInt(this.target.css('borderBottomWidth')) || 0);
			//事件对象(用于绑定移除事件)
			this._fR = (function(zthis){
				return function(event){
					zthis.Resize.apply(zthis,[event||window.event]);
				};
			}(this));
			this._fS = (function(zthis){
				return function(){
					zthis.Stop.apply(zthis,arguments);
				};
			})(this);
			...
			this._mxContainer = this.options.mxContainer;
		  ...
		  Set: function(resize, side) {
			...
			//设置触发对象
			resize.bind('mousedown',(function(zthis){
				return function(event){
					zthis.Start.apply(zthis,[event||window.event].concat(fun));
				};
			})(this));
		  },
		  //准备缩放
		  Start: function(e, fun, touch) {
			...
			this._fun = fun;
			//样式参数值
			this._styleWidth = this.target[0].clientWidth;
			this._styleHeight = this.target[0].clientHeight;
			this._styleLeft = this.target[0].offsetLeft;
			this._styleTop = this.target[0].offsetTop;
			//四条边定位坐标
			...
			$(document).bind('mousemove',this._fR);
			$(document).bind('mouseup',this._fS);
			if(Fantasy.Browser.isMsie){
				this.target.bind('losecapture',this._fS);
		  ...
		  Resize: function(e) {
			...
			this.target.css({width:this._styleWidth + "px",height:this._styleHeight + "px",top:this._styleTop + "px",left:this._styleLeft + "px"})
			this.fireEvent('resize',this);//附加程序
		  },
		  //缩放程序
		  //上
		  Up: function(e) {
		  ...
		  Stop: function() {
			...
			if(Fantasy.Browser.isMsie){
				this.target.unbind('losecapture',this._fS);
