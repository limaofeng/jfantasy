...
dp.sh.Toolbar.CopyToClipboard = function(high) {
	var high = dp.sh.Utils.ParentNode(high),
		highlighter = high.highlighter, 
		code = dp.sh.Utils.FixForBlogger(highlighter.originalCode).replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&');
	window.clipboardData.setData("text", code);
	alert("\u4ee3\u7801\u5df2\u88ab\u590d\u5236\u5230\u526a\u8d34\u677f")
};
dp.sh.Toolbar.PrintSource = function(high) {
	var iframe = document.createElement('IFRAME'),
		doc = null,
		high = dp.sh.Utils.ParentNode(high),
		div = high.highlighter.div;
	iframe.style.cssText = 'position:absolute;width:0px;height:0px;left:-500px;top:-500px;';
	document.body.appendChild(iframe);
	doc = iframe.contentWindow.document;
	dp.sh.Utils.CopyStyles(doc, window.document);
	doc.write('<div class="' + div.className.replace('collapsed', '') + ' printing">' + div.innerHTML + '</div>');
	doc.close();
...
dp.sh.Toolbar.Create = function(highlighter) {
	...
	div.innerHTML = language.charAt(0).toUpperCase()+language.substring(1).toLowerCase() + "\u4ee3\u7801";

	if (window.clipboardData)
		div.innerHTML += ' <a href="javascript:void(0);" onclick="dp.sh.Toolbar.CopyToClipboard(this);return false;" title="\u590d\u5236\u4ee3\u7801"><img style="border:none;" src="'+request.getContextPath()+'/static/js/common/syntaxhighlighter/images/copy.jpg" alt="\u590d\u5236\u4ee3\u7801"/></a>';
	else {
		var code = dp.sh.Utils.FixForBlogger(highlighter.originalCode).replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&');
		div.innerHTML += ' <embed src="'+request.getContextPath()+'/static/js/common/syntaxhighlighter/images/clipboard.swf" width="14" height="15" flashvars=\"clipboard=' + encodeURIComponent(code) + '" quality="high" title="\u590d\u5236\u4ee3\u7801" allowScriptAccess="always" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer"/>';
	}
	div.innerHTML += ' <a href="javascript:void(0);" onclick="dp.sh.Toolbar.PrintSource(this);return false;" title="\u6253\u5370"><img style="border:none;" src="'+request.getContextPath()+'/static/js/common/syntaxhighlighter/images/printer.jpg" alt="\u6253\u5370"/></a>';
	return div;
};
...
dp.sh.Utils.ParentNode = function(high) {
	while (high != null && high.className.indexOf("dp-highlighter") == -1)
		high = high.parentNode;
	return high;
};
dp.sh.RegexLib = {
