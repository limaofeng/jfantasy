...
TouchScroll.prototype={
	...
	parseArgs:function(args){
		...
		if(args && toString.call(args)=='[object Object]')
			for(var key in this._default){
				_d[key]=typeof args[key]==='undefined' ? this._default[key] : toString.call(this._default[key])=='[object Number]' ? parseInt(parseFloat(args[key])*150)/100 : args[key];
			}
	...
	setup:function(){
		...
		this.resize();
		this.addListener(window,'resize',this.bind(this.resize,this),false);
	...
	resize:function(){
		...
		this.element=this.$C('div',{'class':'touchscrollelement','style':';overflow:hidden;width:'+this.scrollWidth+'px;'+'position:absolute;top:-'+this.container.scrollTop+'px;left:-'+this.container.scrollLeft+'px;'});
	...
	refresh:function(flag,show){
		...
		ratio=total==maxLength?0:parseInt(elementOffset/(total-maxLength)*1000)/1060;
