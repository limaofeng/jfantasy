...
public class Weixinpay extends PayProductSupport {
    ...
    public Map<String, String> app(Payment payment, Order order, Properties properties) throws PayException {
        ...
        if ("APP".equals(data.get("trade_type"))) {//APP支付
            ...
            result.put("appid", config.getSellerEmail());
    ...
    public Refund refund(Refund refund) {
        ...
        try {
            ...
            data.put("appid", config.getSellerEmail());
            ...
            //指定读取证书格式为PKCS12 
            KeyStore keyStore = CertUtil.loadKeyStore(loadFileItem(config.getEncryptCert()), config.getBargainorId());
     ...
     * 微信支付 - 统一下单接口
     */
    public Map<String, String> unifiedorder(Payment payment, Order order, Properties prop) {
        PayConfig config = payment.getPayConfig();
        //获取订单信息
        try {
            //组装数据
    ...
    static class Urls {
        ...
        public void setUnifiedorderUrl(String url) {
            this.unifiedorderUrl = url;
        }

        public void setCloseorderUrl(String url) {
