...
<@override name="pageTitle">
    <@s.text name="payment.info.title"/>
<small>
    <@s.text name="payment.info.description"/>
</small>
</@override>
<@override name="head">
<script type="text/javascript">
    $(function(){
        ...
        var deleteMethod = $('.batchDelete').batchExecute($("#allChecked"),$grid.data('grid').pager(),'id','是否确认删除[{name}]？',function(){
            $.msgbox({
                msg : "<@s.text name="payment.info.delete.success"/>",
                type : "success"
            });
        });
    });
...
<div id="searchFormPanel" class="button-panel pad5A">
    <@s.form id="searchForm" namespace="/system/paymentInfo" action="search" method="post">
...
<div class="batch">
    ...
    <a title="<@s.text name="payment.info.button.batchdelete"/>" class="btn small primary-bg batchDelete" href="<@s.url namespace="/system/paymentInfo" action="delete"/>">
        <span class="button-content">
            <i class="glyph-icon icon-trash float-left"></i>
            <@s.text name="payment.info.button.batchdelete"/>
        </span>
    </a>
</div>
<div class="grid-panel">
    <table id="view" class="table table-hover mrg5B text-center">
        ...
        <tr>
            ...
            <th class="sort" orderBy="sn"> <@s.text name="payment.info.list.sn"/></th>
            <th class="sort" orderBy="paymentType"><@s.text name="payment.info.list.paymentType"/></th>
            <th class="sort" orderBy="paymentConfigName"><@s.text name="payment.info.list.paymentConfigName"/></th>
            <th class="sort" orderBy="paymentStatus"><@s.text name="payment.info.list.paymentStatus"/></th>
            <th class="sort" orderBy="totalAmount"><@s.text name="payment.info.list.totalAmount"/></th>
            <th class="text-center"><@s.text name="payment.info.list.actions"/></th>
        </tr>
        </thead>
        <tbody>
        <tr class="template" name="default">
            ...
            <td>{paymentStatus:dict({'ready':'准备','timeout':'超时','invalid':'作废','success':'成功','failure':'失败'})}</td>
            <td>{totalAmount}</td>
            <td class="pad0T pad0B text-center">
                <div class="dropdown actions">
                    ...
                    <ul class="dropdown-menu float-right">
                        <li>
                            <a title="<@s.text name="payment.info.list.actions.view"/>" class="edit" href="<@s.url namespace="/system/paymentInfo" action="view?id={id}"/>" target="after:closest('#page-content')">
                                <i class="glyph-icon icon-external-link-sign mrg5R"></i>
                                <@s.text name="payment.info.list.actions.view"/>
                            </a>
                        </li>
                        <li class="divider"></li>
                        <li>
                            <a href="<@s.url namespace="/system/paymentInfo" action="delete?ids={id}"/>" class="font-red delete" title="<@s.text name="payment.info.list.actions.delete"/>">
                                <i class="glyph-icon icon-remove mrg5R"></i>
                                <@s.text name="payment.info.list.actions.delete"/>
                            </a>
                        </li>
                    </ul>
                </div>
