		...
		public void serialize(Object value, JsonGenerator jgen, SerializerProvider provider) throws IOException {
			...
			if (type.isMapLikeType()) {
				...
				if (mlt.isTrueMapType()) {
					serializer = (JsonSerializer<Object>) buildMapSerializer(config, (MapType) mlt, beanDesc, beanProperty, staticTyping, keySerializer, elementTypeSerializer, elementValueSerializer);
				}
				serializer = (JsonSerializer<Object>) buildMapLikeSerializer(config, mlt, beanDesc, beanProperty, staticTyping, keySerializer, elementTypeSerializer, elementValueSerializer);
			} else if (type.isCollectionLikeType()) {
				CollectionLikeType clt = (CollectionLikeType) type;
				if (clt.isTrueCollectionType()) {
					serializer = (JsonSerializer<Object>) buildCollectionSerializer(config, (CollectionType) clt, beanDesc, beanProperty, staticTyping, elementTypeSerializer, elementValueSerializer);
				} else {
					serializer = (JsonSerializer<Object>) buildCollectionLikeSerializer(config, clt, beanDesc, beanProperty, staticTyping, elementTypeSerializer, elementValueSerializer);
				}
			}
			if (serializer == null) {
