...
public final class Timestamper {
    ...
    private static final int    MAX_LOG = Integer.getInteger("org.jfantasy.framework.hibernate.cache.redis.util.Timestamper.log.max", 1) * 1000;
                ...
                if (VALUE.compareAndSet(current, update)) {
                    if (runs > 1) {
                        log(base, "Thread spin-waits on time to pass. Looped "
                                + "{} times, you might want to increase -Dorg.jfantasy.framework.hibernate.cache.redis.util.Timestamper.shift", runs);
                    }
                    return update;
                }
            }
