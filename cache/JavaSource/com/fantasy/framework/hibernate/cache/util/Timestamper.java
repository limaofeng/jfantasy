...
public final class Timestamper {
    ...
    public static final int BIN_DIGITS = Integer.getInteger("com.fantasy.framework.hibernate.cache.redis.util.Timestamper.shift", 12);

    public static final int ONE_MS = 1 << BIN_DIGITS;

    private static final Logger LOG     = LoggerFactory.getLogger(Timestamper.class);
    private static final int    MAX_LOG = Integer.getInteger("com.fantasy.framework.hibernate.cache.redis.util.Timestamper.log.max", 1) * 1000;
                ...
                if (VALUE.compareAndSet(current, update)) {
                    if (runs > 1) {
                        log(base, "Thread spin-waits on time to pass. Looped "
                                + "{} times, you might want to increase -Dcom.fantasy.framework.hibernate.cache.redis.util.Timestamper.shift", runs);
                    }
                    return update;
                }
            }
